...
return (
  <>
            ...
            <>
              ...
              <Widget
                ...
                props={{
                  ...
                  data: markets.map((row) => row.underlyingAsset
                      ? [
                        <Widget
                        ...
                        />,
                        <div>
                          <div>{Number(row.totalDepositBalanceUSD / row.marketReferencePriceInUsd).toFixed(0)}</div>
                          <div>$ {Number(row.totalDepositBalanceUSD).toFixed(0)}</div>
                        </div>,
                        `${(Number(row.supplyAPY) * 100).toFixed(2)} %`,
                        `${(Number(row.variableBorrowAPY) * 100).toFixed(2)} %`,
                        <div>
                          <div>{Number(row.totalBorrowBalanceUSD / row.marketReferencePriceInUsd).toFixed(0)}</div>
                          <div>$ {Number(row.totalBorrowBalanceUSD).toFixed(0)}</div>
                        </div>,
                        // <VestButton data={row} />,
                      ]
                      : []),
                                  ...
                                  children: row.underlyingAsset ? [
                                    <div className="card-data-row">
                                      ...
                                      <div className="card-data-value">
                                        <div>
                                          {Number(row.totalDepositBalanceUSD / row.marketReferencePriceInUsd).toFixed(0)}
                                        </div>
                                        <div className="card-data-usd-price">$ {Number(row.totalDepositBalanceUSD).toFixed(0)}</div>
                                      </div>
                                    </div>,
                                    <div className="card-data-row">
                                      <div className="card-data-key">
                                    ...
                                    <div className="card-data-row">
                                      ...
                                      <div className="card-data-value">
                                        <div>
                                          {Number(row.totalBorrowBalanceUSD / row.marketReferencePriceInUsd).toFixed(0)}
                                        </div>
                                        <div className="card-data-usd-price">$ {Number(row.totalBorrowBalanceUSD).toFixed(0)}</div>
                                      </div>
                                    </div>,
                                  ] : [],
                                }}
    ...
    {/* {showVestModal && (
      <Widget
        src={`${config.ownerId}/widget/AAVE.Modal.VestModal`}
        props={{
          config,
